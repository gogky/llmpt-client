...
### ~~3.4 · 进度条~~ ✅
- **已完成**：
  - 在 `session_context.py` 的等待循环中动态接入 `huggingface_hub.utils.tqdm`
  - 获取 libtorrent 分文件进度，实时以原生风格渲染进度条
  - 支持通过 HF 原生环境变量透明控制进度条的显示 / 隐藏
  - 在 `monitor.py` 中将啰嗦的周期性 INFO 进度日志降级为 DEBUG
